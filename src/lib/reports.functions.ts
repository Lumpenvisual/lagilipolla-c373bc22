...
function slugify(s: string): string {
  return s.toLowerCase().normalize("NFD").replace(/[\u0300-\u036f]/g, "").replace(/[^a-z0-9]+/g, "-").replace(/(^-|-$)/g, "").slice(0, 32) || "participante";
}
...
export const generateComprobantePDF = createServerFn({ method: "POST" })
  ...
  .handler(async ({ context }) => {
    ...
    const { data: part, error: e1 } = await sb
      .from("participants").select("*").eq("user_id", context.userId).maybeSingle();
    if (e1) throw new Error(e1.message);
    if (!part) throw new Error("No estás inscrito.");
    if (part.estado_pago !== "aprobado") throw new Error("Completa tu pago para descargar el comprobante.");
    ...
    const enc = new TextEncoder().encode(`${part.id}${Math.floor(new Date(myPick.updated_at).getTime() / 1000)}`);
    ...
    const qrDataUrl = await QRCode.toDataURL(`https://lagilipolla.lovable.app/verificar/${codigo}`, { margin: 0, scale: 6 });
    ...
    // Logo / Title
    page.drawText("LA GILIPOLLA 2026", { x: 40, y: A4.h - 55, size: 22, font: helvBold, color: dark });
    page.drawText("Bar El Guanábano - Mundial FIFA 2026", { x: 40, y: A4.h - 72, size: 10, font: helvObl, color: muted });
    page.drawText("COMPROBANTE OFICIAL DE PLANILLA", { x: 40, y: A4.h - 90, size: 9, font: helvBold, color: blue });

    // QR (top right)
    const qrImg = await pdf.embedPng(qrPng);
    const qrSize = 90;
    page.drawImage(qrImg, { x: A4.w - 40 - qrSize, y: A4.h - 30 - qrSize, width: qrSize, height: qrSize });
    page.drawText("Verificar:", { x: A4.w - 40 - qrSize, y: A4.h - 30 - qrSize - 12, size: 7, font: helv, color: muted });
    page.drawText(codigo, { x: A4.w - 40 - qrSize, y: A4.h - 30 - qrSize - 22, size: 8, font: helvBold, color: dark });
    ...
    y -= 14;
    const issued = new Date().toLocaleString("es-CO", { timeZone: "America/Bogota", dateStyle: "long", timeStyle: "short" });
    page.drawText(`Emitido: ${issued} COT`, { x: 40, y, size: 9, font: helv, color: muted });
    y -= 12;
    page.drawText(`Cuota pagada: $${(100000).toLocaleString("es-CO")} COP`, { x: 40, y, size: 9, font: helv, color: muted });
    y -= 18;

    // Groups table
    page.drawLine({ start: { x: 40, y }, end: { x: A4.w - 40, y }, thickness: 0.5, color: line });
    y -= 14;
    page.drawText("PRONOSTICOS POR GRUPO (1° y 2°)", { x: 40, y, size: 10, font: helvBold, color: blue });
    y -= 4;
    ...
    for (const k of GROUP_KEYS) {
      ...
      page.drawText(`1° ${teamName(g, p.pos1)}`, { x: xb, y: yb - 12, size: 8, font: helv, color: dark });
      page.drawText(`2° ${teamName(g, p.pos2)}`, { x: xb, y: yb - 22, size: 8, font: helv, color: dark });
      // points if known
      if (g.pos1 && g.pos2 && p.pos1 && p.pos2) {
        ...
        page.drawText(`+${pts} pts`, { x: xb, y: yb - 33, size: 7, font: helvBold, color: pts > 0 ? blue : muted });
      }
      row++;
    }
    ...
    page.drawLine({ start: { x: 40, y }, end: { x: A4.w - 40, y }, thickness: 0.5, color: line });
    y -= 14;
    page.drawText("PARTIDOS DEL GRUPO K (Colombia)", { x: 40, y, size: 10, font: helvBold, color: red });
    y -= 14;
    for (const m of tournament.group_k_matches) {
      ...
      if (m.gh != null && m.ga != null) {
        page.drawText(`oficial: ${m.gh}-${m.ga}`, { x: A4.w - 130, y, size: 8, font: helvObl, color: muted });
      }
      y -= 13;
    }
    y -= 6;
    ...
    page.drawText(`Goleador del Mundial: ${gol ? `${gol.nombre} (${gol.seleccion})` : "—"}`, { x: 40, y, size: 9, font: helv, color: dark });
    y -= 12;
    page.drawText(`Mejor arquero:        ${arq ? `${arq.nombre} (${arq.seleccion})` : "—"}`, { x: 40, y, size: 9, font: helv, color: dark });
    y -= 18;
    ...
    y -= 9;
    page.drawText(`Marca de tiempo de la planilla: ${new Date(myPick.updated_at).toISOString()}`, { x: 40, y, size: 7, font: helv, color: muted });
    y -= 9;
    page.drawText(`Verifica este comprobante en: https://lagilipolla.lovable.app/verificar/${codigo}`, { x: 40, y, size: 7, font: helv, color: blue });
...
async function makeWorkbook() {
  ...
  // exceljs default-export shim
  const Workbook = (ExcelJS as any).Workbook ?? (ExcelJS as any).default?.Workbook;
  return { wb: new Workbook(), ExcelJS };
}

async function workbookToBase64(wb: any): Promise<string> {
...
export const generateMyPlanillaXlsx = createServerFn({ method: "POST" })
  ...
  .handler(async ({ context }) => {
    const sb = context.supabase;
    const { data: part } = await sb.from("participants").select("*").eq("user_id", context.userId).maybeSingle();
    if (!part) throw new Error("No estás inscrito.");
    ...
    for (const k of GROUP_KEYS) {
      ...
      ws.addRow({ g: k, p1: teamName(g, p.pos1), p2: teamName(g, p.pos2), o1: teamName(g, g.pos1), o2: teamName(g, g.pos2), pts });
    }
...
export const generateParticipantesXlsx = createServerFn({ method: "POST" })
  ...
  .handler(async ({ context }) => {
    await requireAdmin(context as any);
    const sb = context.supabase;
    const { data: parts, error } = await sb.from("participants").select("*").order("inscripcion_at", { ascending: false });
    if (error) throw new Error(error.message);
    const { wb } = await makeWorkbook();
...
export const generateBackupXlsx = createServerFn({ method: "POST" })
  ...
  .handler(async ({ context }) => {
    ...
    const tables = ["participants", "picks", "tournament_state", "user_roles", "admin_audit"] as const;
...
export const verifyComprobante = createServerFn({ method: "POST" })
  .inputValidator(z.object({ code: z.string().min(8).max(64).regex(/^[a-f0-9]+$/) }))
  .handler(async ({ data }) => {
    const { supabaseAdmin } = await import("@/integrations/supabase/client.server");
    const { data: rows, error } = await supabaseAdmin.rpc("get_comprobante_public", { _code: data.code });
    if (error) throw new Error(error.message);
