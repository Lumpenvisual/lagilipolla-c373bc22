...
function DemoTab() {
  ...
  const seed = async () => {
    ...
    toast.success(
      `Planillas demo generadas: ${(data as { picks_demo?: number })?.picks_demo ?? 0}`,
    );
    qc.invalidateQueries({ queryKey: ["polla-leaderboard"] });
    qc.invalidateQueries({ queryKey: ["admin-participants"] });
  };
  ...
  const reset = async () => {
    ...
    toast.success(
      `Demo borrada · ${(data as { auth_users?: number })?.auth_users ?? 0} usuarios eliminados`,
    );
    qc.invalidateQueries({ queryKey: ["polla-leaderboard"] });
    qc.invalidateQueries({ queryKey: ["admin-participants"] });
  };
    ...
    <div className="space-y-6">
      <Card className="border-info/30 bg-card p-6 card-shadow">
        ...
        <p className="mt-2 text-sm text-muted-foreground">
          Hay 6 usuarios demo creados (<code className="text-xs">demo1@gilipolla.co</code>…{" "}
          <code className="text-xs">demo6@gilipolla.co</code>, contraseña{" "}
          <code className="text-xs">Demo2026!</code>) marcados como <strong>[DEMO]</strong> y con
          pago aprobado.
        </p>
        <p className="mt-1 text-sm text-muted-foreground">
          Admin: <code className="text-xs">admin@gilipolla.co</code> · contraseña{" "}
          <code className="text-xs">Guanabano2026!</code>
        </p>
      ...
      <Card className="border-destructive/40 bg-destructive/5 p-6 card-shadow">
        ...
        <p className="mt-2 text-sm text-muted-foreground">
          Elimina los 6 usuarios <strong>[DEMO]</strong>, sus participantes y todas sus planillas.{" "}
          <strong>No</strong> afecta al admin ni a usuarios reales. Usa esto cuando vayas a iniciar
          la polla real.
        </p>
        {!confirmReset ? (
          <Button
          ...
          <div className="mt-4 rounded-lg border border-destructive/50 bg-destructive/10 p-4">
            <div className="flex items-start gap-3">
              ...
              <div className="flex-1">
                ...
                <p className="mt-1 text-sm text-muted-foreground">
                  Esto borra <strong>de forma permanente</strong> a los 6 usuarios demo y sus
                  planillas. No hay vuelta atrás.
                </p>
