...
function AdminPage() {
  ...
  if (loading) return <main className="flex min-h-[60vh] items-center justify-center"><Loader2 className="size-6 animate-spin text-muted-foreground" /></main>;
  if (!user || !isAdmin) {
    return (
      <main className="mx-auto max-w-md px-4 py-16">
        <Card className="border-destructive/40 bg-destructive/5 p-8 text-center card-shadow">
          ...
          <Button className="mt-6" onClick={() => router.navigate({ to: "/" })}>Volver al inicio</Button>
        </Card>
      </main>
    );
  }
        ...
        {tabs.map((tt) => (
          <button key={tt.key} onClick={() => setTab(tt.key)}
            className={`flex items-center gap-2 whitespace-nowrap rounded-lg border px-4 py-2 text-sm font-medium transition-colors ${
              tab === tt.key ? "border-gold bg-gold/15 text-gold" : "border-border bg-card text-muted-foreground hover:text-foreground"
            }`}>
...
function PagosTab() {
  ...
  const { data: parts = [], isLoading } = useQuery({
    ...
    queryFn: async () => {
      const { data, error } = await supabase.from("participants").select("*").order("inscripcion_at", { ascending: false });
  ...
  const setEstado = async (id: string, estado: "aprobado" | "rechazado" | "pendiente") => {
    const { error } = await supabase.from("participants").update({ estado_pago: estado }).eq("id", id);
    if (error) toast.error(error.message); else { toast.success("Actualizado"); qc.invalidateQueries({ queryKey: ["admin-participants"] }); qc.invalidateQueries({ queryKey: ["polla-leaderboard"] }); }
  };
      ...
      <Card className="overflow-x-auto border-border bg-card card-shadow">
        <table className="w-full text-sm">
          <thead><tr className="border-b border-border text-left text-xs uppercase text-muted-foreground">
            <th className="p-3">Nombre</th><th className="p-3">Estado</th><th className="p-3 text-right">Acciones</th>
          </tr></thead>
          <tbody>
            {parts.map((p) => (
              <tr key={p.id} className="border-b border-border/60">
                <td className="p-3 font-medium">{p.nombre}<br /><span className="text-xs text-muted-foreground">{p.email}</span></td>
                <td className="p-3"><span className={`rounded-full border px-2 py-0.5 text-xs ${
                  p.estado_pago === "aprobado" ? "border-success/40 bg-success/10 text-success" :
                  p.estado_pago === "rechazado" ? "border-destructive/40 bg-destructive/10 text-destructive" :
                  "border-gold/40 bg-gold/10 text-gold"}`}>{p.estado_pago}</span></td>
                <td className="p-3 text-right space-x-1">
                  <Button size="sm" variant="hero" disabled={p.estado_pago === "aprobado"} onClick={() => setEstado(p.id, "aprobado")}>✅</Button>
                  <Button size="sm" variant="destructive" disabled={p.estado_pago === "rechazado"} onClick={() => setEstado(p.id, "rechazado")}>❌</Button>
                </td>
              </tr>
            ))}
            {parts.length === 0 && <tr><td colSpan={3} className="p-6 text-center text-muted-foreground">Sin inscritos.</td></tr>}
          </tbody>
        </table>
      </Card>
    </div>
...
function ResultadosTab() {
  ...
  useEffect(() => { if (ts) setDraft(JSON.parse(JSON.stringify(ts))); }, [ts]);
  ...
  const updateGroup = (k: typeof GROUP_KEYS[number], field: "pos1" | "pos2", v: string) => {
    setDraft((d) => d ? { ...d, groups: { ...d.groups, [k]: { ...d.groups[k], [field]: v || null } } } : d);
  };
  ...
  const updateMatch = (id: string, field: "gh" | "ga", v: string) => {
    ...
    setDraft((d) => d ? { ...d, group_k_matches: d.group_k_matches.map((m) => m.id === id ? { ...m, [field]: n } : m) } : d);
  };
  ...
  return (
    <div className="space-y-8">
      <Card className="border-gold/30 bg-card p-5 card-shadow">
        ...
        <p className="mt-1 text-xs text-muted-foreground">Cuando se resuelva un repechaje, edita el nombre del equipo "Ganador Repechaje X".</p>
        <div className="mt-3 space-y-3">
          {GROUP_KEYS.flatMap((k) => draft.groups[k].teams.map((t, i) => t.po ? { k, i, t } : null).filter(Boolean) as { k: typeof GROUP_KEYS[number]; i: number; t: typeof draft.groups[typeof GROUP_KEYS[number]]["teams"][number] }[])
            .map(({ k, i, t }) => (
              <div key={`${k}-${i}`} className="flex items-center gap-2 text-sm">
                ...
                <Input value={t.nombre} onChange={(e) => updateTeam(k, i, e.target.value)} className="h-8" />
              </div>
            ))}
        </div>
      </Card>
            ...
            return (
              <div key={k} className="rounded-lg border border-border bg-muted/30 p-3">
                ...
                <div className="mt-2 space-y-1.5">
                  <select value={g.pos1 ?? ""} onChange={(e) => updateGroup(k, "pos1", e.target.value)} className="w-full rounded-md border border-input bg-background px-2 py-1 text-sm">
                    <option value="">— 1° —</option>
                    {opts.map((o) => <option key={o.id} value={o.id}>{o.label}</option>)}
                  </select>
                  <select value={g.pos2 ?? ""} onChange={(e) => updateGroup(k, "pos2", e.target.value)} className="w-full rounded-md border border-input bg-background px-2 py-1 text-sm">
      ...
      <Card className="border-info/30 bg-card p-5 card-shadow">
        ...
        <div className="mt-4 divide-y divide-border">
          {draft.group_k_matches.map((m) => {
            ...
            const vName = draft.groups.K.teams.find((t) => t.id === m.visitante)?.nombre ?? m.visitante;
            return (
              <div key={m.id} className="flex items-center gap-2 py-2">
                <span className="flex-1 text-right text-sm">{lName}</span>
                <Input type="number" min={0} value={m.gh ?? ""} onChange={(e) => updateMatch(m.id, "gh", e.target.value)} className="h-8 w-14 text-center" />
                <span className="text-muted-foreground">–</span>
                <Input type="number" min={0} value={m.ga ?? ""} onChange={(e) => updateMatch(m.id, "ga", e.target.value)} className="h-8 w-14 text-center" />
      ...
      <Card className="border-destructive/30 bg-card p-5 card-shadow">
        ...
        <div className="mt-4 grid gap-3 sm:grid-cols-2">
          <div>
            ...
            <select value={draft.goleador_id ?? ""} onChange={(e) => setDraft({ ...draft, goleador_id: e.target.value || null })} className="mt-1 w-full rounded-md border border-input bg-background px-2 py-2 text-sm">
              <option value="">— Sin definir —</option>
              {draft.goleadores.map((p) => <option key={p.id} value={p.id}>{p.nombre} · {p.seleccion}</option>)}
            </select>
          </div>
          <div>
            <Label className="text-xs">Mejor arquero</Label>
            <select value={draft.arquero_id ?? ""} onChange={(e) => setDraft({ ...draft, arquero_id: e.target.value || null })} className="mt-1 w-full rounded-md border border-input bg-background px-2 py-2 text-sm">
              <option value="">— Sin definir —</option>
              {draft.arqueros.map((p) => <option key={p.id} value={p.id}>{p.nombre} · {p.seleccion}</option>)}
            </select>
          </div>
        </div>
      </Card>
...
function ListasTab() {
  ...
  useEffect(() => { if (ts) { setGols(ts.goleadores); setArqs(ts.arqueros); } }, [ts]);

  if (!ts) return <Loader2 className="mx-auto size-6 animate-spin text-muted-foreground" />;

  const save = async () => {
    const { error } = await supabase.from("tournament_state").update({
      goleadores: gols as never, arqueros: arqs as never,
    }).eq("id", 1);
    if (error) toast.error(error.message); else { toast.success("Listas guardadas"); qc.invalidateQueries({ queryKey: ["tournament-state"] }); }
  };
  ...
  return (
    <div className="grid gap-6 lg:grid-cols-2">
      ...
      <div className="lg:col-span-2 flex justify-center">
        <Button onClick={save} variant="hero">Guardar listas</Button>
      </div>
    </div>
  );
}

function ListEditor({ title, items, setItems }: { title: string; items: SpecialPlayer[]; setItems: (s: SpecialPlayer[]) => void }) {
  const [n, setN] = useState(""); const [s, setS] = useState("");
  const add = () => {
    if (!n.trim()) return;
    const id = n.toLowerCase().replace(/[^a-z0-9]/g, "").slice(0, 16) + "-" + Math.random().toString(36).slice(2, 6);
      ...
      <ul className="mt-3 divide-y divide-border text-sm">
        {items.map((p, i) => (
          <li key={p.id} className="flex items-center gap-2 py-1.5">
            <span className="flex-1">{p.nombre} <span className="text-xs text-muted-foreground">· {p.seleccion}</span></span>
            <button onClick={() => setItems(items.filter((_, j) => j !== i))} className="text-destructive hover:text-destructive/80"><Trash2 className="size-4" /></button>
          </li>
        ))}
      </ul>
