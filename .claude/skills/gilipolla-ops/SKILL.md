...
- **Especiales:** goleador 10, arquero 10. Comparación **por partes** nombre + selección (`especial_matches` SQL / `especialMatches` TS, migraciones `20260719220000` + `20260720000000`): normaliza mayúsculas/acentos/espacios (`norm_especial`), y acierta por (a) nombre completo igual, o (c) apellido solo/parte del nombre (subconjunto de palabras) con selección presente en AMBOS lados y coincidente (sin selección en alguno = ambiguo, no puntúa). **El typo de escritura del nombre NO puntúa** (regla del admin 19-jul-2026: "Kyllan Mbappé" = 0). Alias `Holanda≡Países Bajos`; typo de selección ≤1 (`Brasill`). **Los oficiales en `tournament_state` deben guardarse en formato canónico `"Nombre (Selección)"`** — en jul-2026 quedaron fuera de formato ("Kylian Mbappé" sin selección) y nadie puntuó hasta corregirlo.
