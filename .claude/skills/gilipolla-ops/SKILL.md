...
- **Códigos vs nombres (display):** `extra_matches.local/visitante` guardan **CÓDIGOS** ISO3 (`RSA`, `CAN`…), igual que `group_k_matches` — el scoring y los helpers de bracket dependen de eso, **no** guardar nombres en los datos. La UI los resuelve a **nombre completo + bandera** con `teamNameByCode(groups, code)` (`src/lib/polla.ts`); aplicado en planilla, leaderboard y reportes Excel. Placeholders (`Ganador Partido N`) se muestran tal cual.
