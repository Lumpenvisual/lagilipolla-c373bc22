...
- **Especiales:** goleador 10, arquero 10. Comparación **por partes** nombre + selección (`especial_matches` SQL / `especialMatches` TS, regla vigente en migración `20260720010000`): normaliza mayúsculas/acentos/espacios (`norm_especial`), y acierta por (a) nombre completo igual, (b) typo pequeño del nombre (levenshtein ≤2) con selección coincidente ("Kyllan Mbappé" vale), o (c) apellido solo/parte del nombre (subconjunto de palabras) con selección presente en AMBOS lados y coincidente (sin selección en alguno = ambiguo, no puntúa). Quien no coincide tiene 0. Alias `Holanda≡Países Bajos`; typo de selección ≤1 (`Brasill`). **Los oficiales en `tournament_state` deben guardarse en formato canónico `"Nombre (Selección)"`** — en jul-2026 quedaron fuera de formato ("Kylian Mbappé" sin selección) y nadie puntuó hasta corregirlo. La card Especiales del admin (Resultados) ya lo garantiza: **campos separados nombre + equipo** (`SpecialEdit`, compone con `composeSpecial`) y **siempre editables** (no hay candado; re-guardar recalcula vía trigger).
