...
const sql = readFileSync(new URL("supabase/migrations/20260608221000_official_data_resolved.sql", root), "utf8");
...
const { error: eUpd } = await sb
  ...
  .eq("id", 1);
if (eUpd) { console.error("❌ UPDATE falló:", eUpd.message); process.exit(1); }
console.log("✅ tournament_state actualizado con datos oficiales");

// Verificación inmediata
const { data: ts } = await sb.from("tournament_state").select("groups, group_k_matches").eq("id", 1).single();
