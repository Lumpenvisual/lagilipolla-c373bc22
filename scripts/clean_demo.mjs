...
const sb = createClient(env.SUPABASE_URL || env.VITE_SUPABASE_URL, env.SUPABASE_PUBLISHABLE_KEY || env.VITE_SUPABASE_PUBLISHABLE_KEY);

const password = process.env.ADMIN_PASS;
if (!password) { console.error("Falta ADMIN_PASS"); process.exit(2); }
const { error: eAuth } = await sb.auth.signInWithPassword({ email: process.env.ADMIN_EMAIL || "admin@gilipolla.co", password });
if (eAuth) { console.error("❌ Login admin falló:", eAuth.message); process.exit(1); }
console.log("✅ Sesión admin iniciada");

const { data, error } = await sb.rpc("reset_polla_demo");
if (error) { console.error("❌ reset_polla_demo falló:", error.message); process.exit(1); }
